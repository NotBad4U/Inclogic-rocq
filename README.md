...
| `rocq-relation-algebra` | (for `KatInc.v` / `KatIncImp.v`) |
...
The `Makefile` is generated by `coq_makefile`/`rocq makefile` from [`_CoqProject`](_CoqProject),
which maps `theories/` to the `IncLogic` logical path:

```sh
rocq makefile -f _CoqProject -o Makefile   # only if Makefile needs regenerating
